...
import { taskAction } from "./stores/slices/task.slice";
          ...
          <div className="listTask">
            {
              taskStore.data?.map((item, index) => {
                return <div className='itemTask' key={item.id}><h3 style={{ textDecoration: checked ? 'line-through' : 'none' }}>{item.name}</h3><div>
                  <i onClick={() => {
                    setChecked(!checked)
